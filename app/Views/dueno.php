    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="data-form" class="pl-3 pr-3">
          <div class="row">
<input type="hidden" id="id_dueno" name="id_dueno" class="form-control" placeholder="Id dueno" maxlength="20" required>
						</div>
						<div class="row">
							<div class="col-md-12">
								<div class="form-group mb-3">
									<label for="nombre_completo" class="col-form-label"> Nombre completo: <span class="text-danger">*</span> </label>
									<input type="text" id="nombre_completo" name="nombre_completo" class="form-control" placeholder="Nombre completo" minlength="0"  maxlength="30" required>
								</div>
							</div>
							<div class="col-md-12">
								<div class="form-group mb-3">
...
<div class="col-md-12">
								<div class="form-group mb-3">
									<label for="fecha_nac" class="col-form-label"> Fecha de nacimiento: <span class="text-danger">*</span> </label>
									<input type="date" id="fecha_nac" name="fecha_nac" class="form-control" dateISO="true" required>
								</div>
							</div>
						</div>
...
<script>
  ...
  $(function() {
    var table = $('#data_table').removeAttr('width').DataTable({
      ...
      "responsive": false,
      "ajax": {
        "url": '<?php echo base_url("/public/dueno/getAll") ?>',
        "type": "POST",
        "dataType": "json",
        async: "true"
      }
  ...
  function save(id_dueno) {
    // reset the form 
    $("#data-form")[0].reset();
    $(".form-control").removeClass('is-invalid').removeClass('is-valid');
    if (typeof id_dueno === 'undefined' || id_dueno < 1) { //add
      urlController = '<?= base_url("/public/dueno/add") ?>';
      ...
      urlController = '<?= base_url("/public/dueno/edit") ?>';
      submitText = '<?= lang("Modificar") ?>';
      $.ajax({
        url: '<?php echo base_url("/public/dueno/getOne") ?>',
        type: 'post',
        data: {
          id_dueno: id_dueno
        },
        ...
        success: function(response) {
          ...
          //insert data to form
			...
			$("#data-form #fecha_nac").val(response.fecha_nac);
  ...
  function remove(id_dueno) {
    Swal.fire({
      title: "<?= lang("Estás a punto de eliminar un cliente") ?>",
      text: "<?= lang("¿Continuar?") ?>",
      icon: 'warning',
      ...
      if (result.value) {
        $.ajax({
          url: '<?php echo base_url("/public/dueno/remove") ?>',
          type: 'post',
          data: {
            id_dueno : id_dueno
          },
